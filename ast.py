...
_stdlib_ast_spec = importlib.util.spec_from_file_location(
    "_python_stdlib_ast",
    os.path.join(sysconfig.get_path("stdlib"), "ast.py"),
)
_stdlib_ast = importlib.util.module_from_spec(_stdlib_ast_spec)
_stdlib_ast_spec.loader.exec_module(_stdlib_ast)

AST = _stdlib_ast.AST
NodeVisitor = _stdlib_ast.NodeVisitor
iter_child_nodes = _stdlib_ast.iter_child_nodes
walk = _stdlib_ast.walk
parse = _stdlib_ast.parse
literal_eval = _stdlib_ast.literal_eval
dump = _stdlib_ast.dump
get_docstring = _stdlib_ast.get_docstring

class BaseBox(object):
    pass

# AST node definitions for the Sarana language (plain Python runtime).
...
class Dict(BaseBox):
    ...
    def rep(self):
        result = 'Dict('
        result += ",".join(self.map(lambda k: "%s: %s" % (k[0].rep(), k[1].rep()),self.data.items()))
        result += ')'
        return result
    
    def to_string(self):
        return '{ %s }' % (", ".join(self.map(lambda k: "%s: %s" % (k[0].to_string(), k[1].to_string()),self.values.items())))
