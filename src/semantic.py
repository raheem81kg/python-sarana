...
class SemanticAnalyzer:
    ...
    def __init__(self):
        self.symbol_table = SymbolTable()  # global scope
    ...
    def visit(self, node):
        ...
        if method is None:
            # Node type not implemented — this is okay for nodes that
            # don't need semantic checking (like literals)
            return
        return method(node)
    ...
    def visit_TryKetchStatement(self, node):
        ...
        """
        # Mark that we're inside a try block (for lenient error handling)
        was_in_try = getattr(self, 'in_try_block', False)
    ...
    def visit_BinaryOp(self, node):
        ...
        # Skip this check if we're inside a try block (intentional error demonstration)
        if node.operator in ('/', '%'):
            if isinstance(node.right, Integer) and node.right.value == 0:
                if not getattr(self, 'in_try_block', False):
                    self.add_error(
    ...
    def visit_UnaryOp(self, node):
        ...
        if node.operator == '-':
            if isinstance(node.operand, String):
                self.add_error(
                    "Cannot negate a string with unary minus",
                    line=node.line
                )
